...
public  class HiveHelper {
    ...
    public void parquetFileStartWrite(RecordReceiver lineReceiver, Configuration config, String fileName,
        ...
        FileOutputFormat outFormat = new MapredParquetOutputFormat();

        Properties tableProps = createTableProperties(columnNames, columnTypeInspectors);

        //使用官方指定的SnappyCodec压缩(因为是老版本，所以需要从源码里找)
        if("SNAPPY".equalsIgnoreCase(compress)){
            //outFormat.setOutputCompressorClass(conf, parquet.hadoop.codec.SnappyCodec.class);
        }else if("GZIP".equalsIgnoreCase(compress)){
            outFormat.setOutputCompressorClass(conf, org.apache.hadoop.io.compress.GzipCodec.class);
        }else {
            LOG.warn(String.format("目前不支持您配置的 compress 模式 : [%s]， 默认使用未压缩", compress));
        }

        try {
            //参数isCompressed和Text.class没有意义
            FileSinkOperator.RecordWriter recordWriter = ((MapredParquetOutputFormat) outFormat).getHiveRecordWriter(conf, outputPath, Text.class
                    , false, tableProps, Reporter.NULL);
            Record record = null;
            while ((record = lineReceiver.getFromReader()) != null) {
                MutablePair<List<Object>, Boolean> transportResult =  transportOneRecord(record,columns,taskPluginCollector);
                if (!transportResult.getRight()) {
                    Writable parquetRecord = serde.serialize(transportResult.getLeft(), inspector);
                    recordWriter.write(parquetRecord);
                }
            }
    ...
    private static Properties createTableProperties(List<String> columnNames, List<ObjectInspector> objectInspectors)
    {
        ...
        orderTableProperties.setProperty("columns.types", Joiner.on(',').join(transform(objectInspectors, new Function<ObjectInspector, Object>() {
            @Override
            public Object apply(@Nullable ObjectInspector objectInspector) {
                    ...
                    try {
                        switch (columnType) {
                            ...
                            case DATE:
                                ...
                                break;
                            case TIMESTAMP:
                                recordList.add(new Timestamp(column.asDate().getTime()));
                                break;
                            default:
                                throw DataXException
                                        .asDataXException(
