    ...
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            ...
            if( !fileType.equalsIgnoreCase("ORC") && !fileType.equalsIgnoreCase("TEXT") && !fileType.equalsIgnoreCase("PARQUET")){
                String message = "HiveWriter插件目前只支持ORC、TEXT和PARQUET两种格式的文件,请将filetype选项的值配置为ORC、TEXT和PARQUET";
            ...
            //jdbc_url
            this.jdbc_url = this.writerSliceConfig.getNecessaryValue(Key.JDBC_URL,HiveWriterErrorCode.REQUIRED_VALUE);
            //username
            this.username = this.writerSliceConfig.getNecessaryValue(Key.USERNAME,HiveWriterErrorCode.REQUIRED_VALUE);
            //password
            this.password = this.writerSliceConfig.getNecessaryValue(Key.PASSWORD,HiveWriterErrorCode.REQUIRED_VALUE);
            //table_name
            this.table_name = this.writerSliceConfig.getNecessaryValue(Key.TABLE_NAME,HiveWriterErrorCode.REQUIRED_VALUE);
            //columns check
            ...
            if (!supportedWriteModes.contains(writeMode)) {
                throw DataXException.asDataXException(HiveWriterErrorCode.ILLEGAL_VALUE,
                        String.format("支持append, nonConflict, truncate三种模式, 不支持您配置的 writeMode 模式 : [%s]",
                                writeMode));
            }
            ...
            if(fileType.equalsIgnoreCase("TEXT")){
            ...
            }else if(fileType.equalsIgnoreCase("ORC") || fileType.equalsIgnoreCase("PARQUET")){
                ...
                if(null == compress){
                    ...
                    if(!orcSupportedCompress.contains(compress)){
                        throw DataXException.asDataXException(HiveWriterErrorCode.ILLEGAL_VALUE,
                                String.format("目前ORC FILE和PARQUET FILE仅支持SNAPPY压缩, 不支持您配置的 compress 模式 : [%s]",
                                        compress));
                    }
                }
            }
...
//                        String.format("您配置的path: [%s] 不存在, 请先在hive端创建对应的数据库和表.", path));
                LOG.info(String.format("您配置的path: [%s] 不存在,自动为您创建此分区",path));
                hiveHelper.addPartition(path,table_name,jdbc_url,username,password);
            }
        }
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWrite(RecordReceiver lineReceiver) {
            ...
            if(fileType.equalsIgnoreCase("TEXT")){
                ...
                //写PARQUET FILE
                hiveHelper.parquetFileStartWrite(lineReceiver,this.writerSliceConfig, this.fileName,
                        this.getTaskPluginCollector());
            }
